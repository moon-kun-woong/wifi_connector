"""
SMS 메시지 전송을 위한 모듈 - 이메일을 통한 SMS 게이트웨이 활용
...
def send_sms(phone_number: str, message: str, carrier: Optional[str] = None) -> bool:
    """
    ...
    """
    # 로그 기록
    logger.info(f"[SMS 발송 시도] 수신자: {phone_number}, 내용: {message}")
    
    # SMTP 설정이 비어있는 경우 로깅만 수행
    if not SMTP_USERNAME or not SMTP_PASSWORD:
        logger.warning("[SMS 발송 실패] SMTP 계정 정보가 설정되지 않았습니다. 환경 변수를 확인하세요.")
    ...
    try:
        ...
        if carrier not in CARRIER_MAP:
            logger.warning(f"[SMS 발송 실패] 알 수 없는 통신사 코드: {carrier}, 기본값 사용")
        ...
        try:
            ...
            logger.error(f"[SMS 발송 실패] SMTP 인증 오류: {str(e)}")
            return False
        except smtplib.SMTPException as e:
            logger.error(f"[SMS 발송 실패] SMTP 오류: {str(e)}")
            return False
        except Exception as e:
            logger.error(f"[SMS 발송 실패] 기타 예외 발생: {str(e)}")
            return False
            
        logger.info(f"[SMS 발송 성공] 수신자: {phone_number} (통신사: {carrier})")
        return True
        
    except smtplib.SMTPException as e:
        logger.exception(f"[SMS 발송 실패] SMTP 오류: {str(e)}")
        return False
    except Exception as e:
        logger.exception(f"[SMS 발송 실패] 예상치 못한 오류: {str(e)}")
        return False
